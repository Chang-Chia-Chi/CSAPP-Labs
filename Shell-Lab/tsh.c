...
int parseline(const char *cmdline, char **argv) 
{
    ...
    while (delim) { /* while hasn't reached end of cmd line */
	argv[argc++] = buf; /* assign argument */
	*delim = '\0'; /* set null terminator at end of argument */
	buf = delim + 1; /* get next character */
	while (*buf && (*buf == ' ')) /* ignore spaces */
	       buf++;

	if (*buf == '\'') { /* get pos for first white space in rest of string */
	    buf++;
	    delim = strchr(buf, '\'');
	}
	else {
